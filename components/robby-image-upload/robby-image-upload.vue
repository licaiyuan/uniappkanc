<template>
	<view class="imageUploadContainer">
		<view class="imageUploadList">
			<view class="imageItem" v-bind:key="index" v-for="(item, index) in imageList">
				<image :src="item.src" :class="{ dragging: isDragging(index) }" draggable="false" @tap="previewImage(imageList, index)" :data-index="index"></image>
				<view v-if="isShowDel" class="imageDel" @tap="deleteImage(item, index)" :data-index="index">x</view>
			</view>
			<view v-if="isShowDel" class="imageUpload" @tap="selectImage">+</view>
		</view>
		<image v-if="showMoveImage" class="moveImage" :style="{ left: posMoveImageLeft, top: posMoveImageTop }" :src="moveImagePath"></image>
		<view class="yltp" v-if="ylshow" @click="ylshow = !ylshow"><image :src="yltpsrc"></image></view>
	</view>
</template>
...
export default {
	...
	watch: {
		value(newVal, oldVal) {
			this.imageList = this.clfileid(this.value);
	...
	mounted: function() {
		...
		this.clfileid(this.value);
		this.qqurl = this.serverUrl + '/info-surveys/bindImg?moduleId=' + this.gdid + '&type=' + this.typeid;
	...
	methods: {
		...
		clfileid(val) {
			val.forEach(item => {
				this.imageList.push({ src: this.serverUrl + '/fs/files/' + item + '/0', fileId: item });
			});
		...
		selectImage: function() {
			...
			uni.chooseImage({
				count: _self.limit ? _self.limit - _self.imageList.length : 1,
				success: function(e) {
					...
					if (_self.serverUrl) {
						...
						Promise.all(promiseWorkList).then(result => {
							...
							_self.clfileid([fileId]);
							// _self.imageList.push({ src: _self.serverUrl + '/fs/files/' + fileId + '/0', fileId });
							// console.log.log(_self.imageList)
		...
		deleteImage(val, index) {
			...
			if (this.serverUrlDeleteImage) {
				uni.request({
					url: this.serverUrl + '/file-modules/' + val.fileId,
					method: 'DELETE',
		...
		previewImage(item, index) {
			...
			uni.previewImage({
				current: item[index].src,
				indicator: 'number',
				loop: 'true',
				urls: urls
			});
			// this.ylshow=true
			// this.yltpsrc=item.src
			// uni.previewImage({
			// 	urls: [item.src],
			// 	longPressActions: {
