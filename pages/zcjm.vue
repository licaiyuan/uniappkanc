<template>
	<view class="ztbjs">
		...
		<view class="inputGroup">
			...
			<view class="anjz" style="text-align: center;">
				<button type="primary" @click="login" class="login">立即注册</button>
		<!-- 		<text class="ljzc">*平台承诺：所有用户都可为我司提供勘察服务，完成后即可得到相应奖励</text> -->
			</view>
		</view>
...
import { back } from './index/common.js';
export default {
	// mixins: [toggle],
	data() {
		return {
	...
	methods: {
		...
		async login() {
			...
			let {
				data: { code }
			} = await this.$api.registe({ username: _this.username, password: _this.password });
			if (code == 200) {
				this.$tcts('注册成功');
				// back()
			} else {
